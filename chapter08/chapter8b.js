/* #############################################################
CHAPTER 8b: 2D View & Translation Matrix
...
function setupGUI(canvas, render) {
    ...
    pointAFolder.add(state, "aX", 0, canvas.width).name("X").onChange(render);
    pointAFolder.add(state, "aY", 0, canvas.height).name("Y").onChange(render);
    // Point B
    const pointBFolder = verticesFolder.addFolder("Point B");
    pointBFolder.add(state, "bX", 0, canvas.width).name("X").onChange(render);
    pointBFolder.add(state, "bY", 0, canvas.height).name("Y").onChange(render);
    // Point C
...
function main() {
    ...
    const locationUniformProjection = gl.getUniformLocation(program, "u_projection");
    const locationUniformView = gl.getUniformLocation(program, "u_view");
    ...
    function render() {
        ...
        // BACKGROUND
        //gl.clearColor(0.39, 0.33, 0.58, 1.0); //Sckorpio-Purple
        ...
        // ---------------------------------------------------------
        // projection matrix
        const projection = mat3.create(); // Identity
        // view matrix
        const view = mat3.fromValues(
        ...
        gl.uniformMatrix3fv(locationUniformView,false,view);
